package dk.sebb.burstLib.misc
{
	...
	public class InfoBox extends Sprite
	{
		...
		public function convo(id:String, pause:Boolean = true):void {
			if(TMXLevelModel.instance.data.conversations[id]) {
				hasConvo = true;
				visible = true;
				currentConvo = id;
				currentConvoIndex = 0;
		...
		public function convoNext():void {
			if(currentConvo !== "" && TMXLevelModel.instance.data.conversations[currentConvo]) {
				var stmtnt:Object = TMXLevelModel.instance.data.conversations[currentConvo].statements[currentConvoIndex];
				if(stmtnt) {
					hasConvo = true;
					write(TMXLevelModel.instance.data.people[stmtnt.person].name + ": " + stmtnt.text + "\n");
					currentConvoIndex++;
				} else {
					clear();
				}
